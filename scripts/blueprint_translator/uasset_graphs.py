...
DEVKIT_PATH_MAPPINGS_FILE = PROJECT_ROOT / "devkit_path_mappings.txt"
...
AUTHORITATIVE_TARGET_PIN_CANDIDATE_METHODS = frozenset(
...
def normalize_blueprint_object_path(raw_text: str) -> str:
    ...
    if path_match:
        ...
        shorthand_match = re.search(r"(?P<path>(?:/?Game|/?Mods)/[^\s,'\"]+|[A-Za-z0-9_][\w.\-]*/[^\s,'\"]+)", text)
        if shorthand_match:
...
def _clean_path_text(value: str | os.PathLike[str]) -> str:
...
def _split_path_list(value: str | None) -> list[str]:
    if not value:
        return []
    return [_clean_path_text(item) for item in value.split(os.pathsep) if _clean_path_text(item)]


def _normalize_mount_path(value: str) -> str:
    mount = str(value or "").strip().replace("\\", "/").strip("\"'")
    if not mount:
        return ""
    if not mount.startswith("/"):
        mount = "/" + mount
    mount = re.sub(r"/+", "/", mount).rstrip("/")
    if mount.lower() == "/game":
        return "/Game"
    if mount.lower().startswith("/game/"):
        return "/Game/" + mount[6:]
    return mount


def _dedupe_paths(roots: Iterable[Path]) -> list[Path]:
    unique: list[Path] = []
    seen: set[str] = set()
    for root in roots:
        key = str(root).lower()
...
def _parse_mount_mapping(value: str) -> tuple[str, Path] | None:
    if "=" not in value:
        return None
    mount_text, root_text = value.split("=", 1)
    mount = _normalize_mount_path(mount_text)
    root = _clean_path_text(root_text)
    if not mount or not root:
        return None
    return mount, Path(root).expanduser()


def _mod_mount_for_content_root(root: Path) -> tuple[str, Path] | None:
    parts = [part.lower() for part in root.parts]
    if root.name.lower() == "content" and len(root.parts) >= 3 and root.parent.parent.name.lower() == "mods":
        return f"/Game/Mods/{root.parent.name}", root
    if root.parent.name.lower() == "mods":
        return f"/Game/Mods/{root.name}", root / "Content"
    if "mods" in parts:
        index = parts.index("mods")
        if len(root.parts) > index + 1:
            mod_name = root.parts[index + 1]
            if root.name.lower() == "content":
                return f"/Game/Mods/{mod_name}", root
    return None


def path_mappings(extra_roots: Iterable[str | os.PathLike[str]] | None = None) -> list[tuple[str, Path]]:
    mappings: list[tuple[str, Path]] = []
    for env_name in ("ARK_DEVKIT_PATH_MAPPINGS", "BLUEPRINT_TO_CODE_DEVKIT_PATH_MAPPINGS"):
        for value in _split_path_list(os.environ.get(env_name)):
            parsed = _parse_mount_mapping(value)
            if parsed:
                mappings.append(parsed)
    for line in _config_lines(DEVKIT_PATH_MAPPINGS_FILE):
        parsed = _parse_mount_mapping(line)
        if parsed:
            mappings.append(parsed)
    for root in content_roots(extra_roots):
        parsed = _mod_mount_for_content_root(root)
        if parsed:
            mappings.append(parsed)

    unique: list[tuple[str, Path]] = []
    seen: set[tuple[str, str]] = set()
    for mount, root in mappings:
        key = (mount.lower(), str(root).lower())
        if key in seen:
            continue
        seen.add(key)
        unique.append((mount, root))
    return unique


def _path_under_mount(package_path: str, mount: str, root: Path) -> Path | None:
    package_lower = package_path.lower()
    mount_lower = mount.lower()
    if package_lower == mount_lower:
        return None
    if not package_lower.startswith(mount_lower + "/"):
        return None
    relative = package_path[len(mount) :].lstrip("/")
    if not relative:
        return None
    return root / Path(*relative.split("/")).with_suffix(".uasset")


def _external_mod_content_candidate(package_path: str, content_root: Path) -> Path | None:
    relative = package_path.removeprefix("/Game/")
    parts = relative.split("/")
    if len(parts) < 3 or parts[0].lower() != "mods":
        return None
    mod_name = parts[1]
    rest = parts[2:]
    if not rest:
        return None
    if content_root.name.lower() == "content" and content_root.parent.parent.name.lower() == "mods":
        mod_content_root = content_root
    elif content_root.parent.name.lower() == "mods":
        mod_content_root = content_root / "Content"
    else:
        shooter_game_root = content_root.parent if content_root.name.lower() == "content" else content_root
        mod_content_root = shooter_game_root / "Mods" / mod_name / "Content"
    return mod_content_root / Path(*rest).with_suffix(".uasset")


def _content_root_candidates(package_path: str, content_root: Path) -> list[Path]:
    relative = package_path.removeprefix("/Game/") + ".uasset"
    relative_path = Path(*relative.split("/"))
    candidates = [content_root / relative_path]
    mod_candidate = _external_mod_content_candidate(package_path, content_root)
    if mod_candidate:
...
def object_path_to_uasset_path(
    ...
    def try_candidate(candidate: Path) -> Path | None:
...
def parse_uasset_exports(
    ...
    try:
        old_export_layout = int(summary.get("file_version_ue4") or 0) < 500
        for index in range(count):
            base = offset + index * size
            if old_export_layout:
                class_index = _read_i32(data, base)
                super_index = _read_i32(data, base + 4)
                template_index = 0
                outer_index = _read_i32(data, base + 8)
                object_name, _name_index, object_number = _read_fname(names, data, base + 12)
                object_flags = _read_u32(data, base + 20)
                serial_size = _read_i32(data, base + 24)
                serial_offset = _read_i32(data, base + 28)
            else:
                class_index = _read_i32(data, base)
                super_index = _read_i32(data, base + 4)
...
def extract_object_ref_array(
    ...
    if len(block) >= 4:
        count = _read_i32(block, 0)
        refs_start = 4
        refs_end = refs_start + count * 4
        if count >= 0 and refs_end == len(block):
            refs = [_read_i32(block, refs_start + index * 4) for index in range(count)]
            if all(value == 0 or _valid_package_index(value, imports, exports) for value in refs):
                return refs, 0

    declared_size = _read_i32(block, 16) if len(block) >= 20 else 0
    if 0 < declared_size <= len(block):
        for count_offset in (32, 36, 37, 40, 28, 24):
            if count_offset + 4 > len(block):
                continue
            count = _read_i32(block, count_offset)
            if count < 0 or count > 100_000:
                continue
            refs_start = count_offset + 4
            refs_end = refs_start + count * 4
            if refs_end > len(block):
                continue
            if declared_size and 4 + count * 4 != declared_size:
                continue
            refs = [_read_i32(block, refs_start + index * 4) for index in range(count)]
            if all(value == 0 or _valid_package_index(value, imports, exports) for value in refs):
                return refs, count_offset
...
def array_parse_payload(refs: list[int], array_offset: int, raw_size: int) -> dict[str, object]:
    return {
        "parsed": bool(refs),
        "element_kind": "FPackageIndex" if refs else "unknown",
        "raw_size": max(0, int(raw_size)),
...
def is_member_reference_name_candidate(value: str) -> bool:
    ...
    if value in {"None", "MemberName", "MemberParent", "MemberReference", "ObjectProperty", "NameProperty"}:
        return False
    if value.startswith(("/Game/", "/Script/", "/Engine/")) or "/" in value or "\\" in value:
        return False
    return True
...
def extract_member_reference_name(data: bytes, names: list[str]) -> str:
    for pos, _name in fname_positions(data, names, {"MemberName"}):
        value = _read_fname_candidate(data, names, [pos + 24, pos + 25, pos + 26, pos + 29])
...
def _ark_cdo_property_tag_at(
    export_data: bytes,
    names: list[str],
    pos: int,
    limit: int,
) -> dict[str, object] | None:
    """Parse the compact tagged-property layout used by current ARK DevKit assets.

    The old implementation searched every byte for ``FName + property type`` and
    therefore promoted fields nested inside struct arrays to top-level defaults.
    This parser advances by the serialized size recorded in each property tag so
    nested values retain their owning array/struct boundary.
    """

    if pos < 0 or pos + 24 > limit:
        return None
    ...
    if (
        ...
        or type_name not in UOBJECT_PROPERTY_TYPE_NAMES
    ):
        return None

    declared_size = _read_i32(export_data, pos + 16)
    array_index = _read_i32(export_data, pos + 20)
    if declared_size < 0 or declared_size > limit - pos or array_index < 0:
        return None
    fixed_sizes = {
        "ObjectProperty": 4,
        "IntProperty": 4,
    ...
    expected_size = fixed_sizes.get(type_name)
    ...
    if type_name == "ArrayProperty":
        inner_info = _fname_at(export_data, pos + 24, names)
        if not inner_info or inner_info[0] not in UOBJECT_PROPERTY_TYPE_NAMES:
            return None
        metadata["inner_type"] = inner_info[0]
        value_offset = pos + 32
    elif type_name == "StructProperty":
        struct_info = _fname_at(export_data, pos + 24, names)
        if not struct_info:
            return None
        ...
        if not enum_info:
            return None
        metadata["enum"] = enum_info[0]
        value_offset = pos + 32
    elif type_name == "BoolProperty":
        # UE serializes the bool in the tag while its declared payload size is 0.
        value_offset = pos + 24

    value_size = max(1, declared_size) if type_name == "BoolProperty" else declared_size
    property_end = value_offset + value_size
    if property_end > limit:
        return None
    if type_name == "ArrayProperty":
        if declared_size < 4:
            return None
...
def _ark_cdo_property_sequence(
    export_data: bytes,
    names: list[str],
    *,
    start: int = 0,
    ...
    sequence_limit = len(export_data) if limit is None else min(len(export_data), max(start, limit))
    blocks: list[dict[str, object]] = []
    cursor = start
    while cursor < sequence_limit:
        name_info = _fname_at(export_data, cursor, names)
        if name_info and name_info[0] == "None":
            return blocks, cursor + 8, True
        block = _ark_cdo_property_tag_at(export_data, names, cursor, sequence_limit)
        if not block:
            return blocks, cursor, False
        blocks.append(block)
        next_cursor = int(block["end"])
        if next_cursor <= cursor:
            return blocks, cursor, False
        cursor = next_cursor
    return blocks, cursor, cursor == sequence_limit


def _ark_guid_cdo_property_tag_at(
    export_data: bytes,
    names: list[str],
    pos: int,
    limit: int,
) -> dict[str, object] | None:
    """Parse ARK compact tags that include a property-guid marker byte."""

    if pos < 0 or pos + 25 > limit:
        return None
    name_info = _fname_at(export_data, pos, names)
    type_info = _fname_at(export_data, pos + 8, names) if name_info else None
    if not name_info or not type_info:
    ...
    if type_name == "ArrayProperty":
        ...
        if not inner_info or inner_info[0] not in UOBJECT_PROPERTY_TYPE_NAMES:
            return None
        metadata["inner_type"] = inner_info[0]
        marker_offset = pos + 32
    elif type_name == "StructProperty":
        struct_info = _fname_at(export_data, pos + 24, names)
        if not struct_info:
            return None
        metadata["struct"] = struct_info[0]
        marker_offset = pos + 48
    elif type_name in {"ByteProperty", "EnumProperty"}:
        enum_info = _fname_at(export_data, pos + 24, names)
        if not enum_info:
            return None
    ...
    if marker_offset >= limit:
        return None
    if type_name == "BoolProperty":
        # Compact ARK bools store the bool byte in the tag, followed by the
        # same property-GUID marker used by other property types.  Stopping at
        # the bool byte shifts the next tag by one byte and can make the
        # fallback scanner attach a later scalar to the wrong payload.
        value_offset = marker_offset
        guid_marker_offset = marker_offset + 1
        if guid_marker_offset >= limit:
            return None
        marker = export_data[guid_marker_offset]
        if marker not in {0, 1}:
            return None
        property_guid_offset = guid_marker_offset + 1 if marker else None
        property_end = guid_marker_offset + 1 + (16 if marker else 0)
    else:
        marker = export_data[marker_offset]
        if marker not in {0, 1}:
            return None
        property_guid_offset = marker_offset + 1 if marker else None
        value_offset = marker_offset + 1 + (16 if marker else 0)
        property_end = value_offset + declared_size
    if property_end > limit:
        return None
    if property_guid_offset is not None:
        metadata["property_guid_offset"] = property_guid_offset
    if type_name == "ArrayProperty":
        if declared_size < 4:
            return None
        count = _read_i32(export_data, value_offset)
...
def _ark_guid_cdo_property_sequence(
    export_data: bytes,
    names: list[str],
    *,
    start: int = 0,
    ...
    while cursor < sequence_limit:
        ...
        if name_info and name_info[0] == "None":
            return blocks, cursor + 8, True
        block = _ark_guid_cdo_property_tag_at(export_data, names, cursor, sequence_limit)
        if not block:
            return blocks, cursor, False
        blocks.append(block)
        next_cursor = int(block["end"])
...
def _ue5_property_type_name_at(
    export_data: bytes,
    names: list[str],
    pos: int,
    limit: int,
    *,
    ...
    """Read UE5's recursive FPropertyTypeName descriptor."""

    if depth > 8 or pos + 12 > limit:
        return None
    name_info = _fname_at(export_data, pos, names)
    if not name_info:
        return None
    parameter_count = _read_i32(export_data, pos + 8)
    if parameter_count < 0 or parameter_count > 16:
        return None
    cursor = pos + 12
    parameters: list[dict[str, object]] = []
    for _index in range(parameter_count):
        parsed = _ue5_property_type_name_at(export_data, names, cursor, limit, depth=depth + 1)
        if not parsed:
            return None
        parameter, cursor = parsed
        parameters.append(parameter)
    return {"name": name_info[0], "parameters": parameters}, cursor


def _ue5_cdo_property_tag_at(
    export_data: bytes,
    names: list[str],
    pos: int,
    limit: int,
    ...
    if (
        ...
        return None
    parsed_type = _ue5_property_type_name_at(export_data, names, pos + 8, limit)
    if not parsed_type:
        return None
    descriptor, cursor = parsed_type
    type_name = str(descriptor.get("name") or "")
    if type_name not in UOBJECT_PROPERTY_TYPE_NAMES or cursor + 4 > limit:
        return None
    declared_size = _read_i32(export_data, cursor)
    if declared_size < 0 or declared_size > limit - cursor:
        return None
    cursor += 4
    ...
    if type_name == "BoolProperty":
        if cursor >= limit:
            return None
        value_offset = cursor
        property_end = cursor + 1
        tag_flags = 0
    else:
        if cursor >= limit:
            return None
        # UE5 stores an extension/serialization flags byte between Size and
        # Value. Struct values in current ARK assets commonly use 0x08 while
        # scalar and array values use 0x00.
        tag_flags = int(export_data[cursor])
        cursor += 1
        property_guid_offset: int | None = None
        if tag_flags & 0x02:
            if cursor + 16 > limit:
                return None
            property_guid_offset = cursor
            cursor += 16
        value_offset = cursor
        property_end = value_offset + declared_size
    if property_end > limit:
    ...
    parameter_rows = parameters if isinstance(parameters, list) else []
    metadata: dict[str, object] = {"type_descriptor": descriptor}
    if type_name != "BoolProperty" and property_guid_offset is not None:
        metadata["property_guid_offset"] = property_guid_offset
    if type_name == "ArrayProperty":
        if declared_size < 4 or not parameter_rows:
            return None
        inner_type = str(parameter_rows[0].get("name") or "") if isinstance(parameter_rows[0], dict) else ""
        if inner_type not in UOBJECT_PROPERTY_TYPE_NAMES:
            return None
        count = _read_i32(export_data, value_offset)
        if count < 0 or count > 1_000_000:
            return None
        ...
        if not parameter_rows or not isinstance(parameter_rows[0], dict):
            return None
        metadata["struct"] = str(parameter_rows[0].get("name") or "")
...
def _ue5_cdo_property_sequence(
    export_data: bytes,
    names: list[str],
    *,
    start: int = 0,
    ...
    while cursor < sequence_limit:
        ...
        if name_info and name_info[0] == "None":
            return blocks, cursor + 8, True
        block = _ue5_cdo_property_tag_at(export_data, names, cursor, sequence_limit)
        if not block:
            return blocks, cursor, False
        blocks.append(block)
        next_cursor = int(block["end"])
...
def _scan_ue5_cdo_property_blocks(export_data: bytes, names: list[str]) -> list[dict[str, object]]:
    """Find outer UE5 tagged values when native serialization wraps tag streams."""

    candidates: list[dict[str, object]] = []
    for pos in range(0, max(0, len(export_data) - 24)):
        block = _ue5_cdo_property_tag_at(export_data, names, pos, len(export_data))
        if block:
    ...
    covered_until = -1
    for block in sorted(candidates, key=lambda row: (int(row["offset"]), -int(row["end"]))):
        start = int(block["offset"])
        if start < covered_until:
            continue
        selected.append(block)
        covered_until = int(block["end"])
    return selected


def _scan_legacy_cdo_property_blocks(export_data: bytes, names: list[str]) -> list[dict[str, object]]:
    """Compatibility scan for older tag layouts without flattening known structured blocks."""
...
def _sequence_ends_at_export_or_zero_padding(export_data: bytes, end: int) -> bool:
    """Accept a verified None terminator plus ARK's bounded alignment padding."""

    if end == len(export_data):
        return True
    if end < 0 or end > len(export_data):
        return False
    trailing = export_data[end:]
    return 0 < len(trailing) <= 16 and not any(trailing)


def cdo_property_tag_blocks(export_data: bytes, names: list[str]) -> list[dict[str, object]]:
    compact_blocks, compact_end, compact_ok = _ark_cdo_property_sequence(export_data, names)
    if (
        compact_ok
        and compact_blocks
        and _sequence_ends_at_export_or_zero_padding(export_data, compact_end)
    ):
        return compact_blocks

    guid_blocks, guid_end, guid_ok = _ark_guid_cdo_property_sequence(export_data, names)
    if (
        guid_ok
        and guid_blocks
        and _sequence_ends_at_export_or_zero_padding(export_data, guid_end)
    ):
        return guid_blocks

    ue5_blocks, ue5_end, ue5_ok = _ue5_cdo_property_sequence(export_data, names)
    if (
        ue5_ok
        and ue5_blocks
        and _sequence_ends_at_export_or_zero_padding(export_data, ue5_end)
    ):
        return ue5_blocks

    scanned_ue5 = _scan_ue5_cdo_property_blocks(export_data, names)
    legacy_blocks = _scan_legacy_cdo_property_blocks(export_data, names)
    if scanned_ue5:
        structured_ranges = [(int(block["offset"]), int(block["end"])) for block in scanned_ue5]
        uncovered_legacy = [
            block
            for block in legacy_blocks
            if not any(
                int(block["offset"]) < structured_end and int(block["end"]) > structured_start
                for structured_start, structured_end in structured_ranges
            )
        ]
        return sorted(scanned_ue5 + uncovered_legacy, key=lambda block: int(block["offset"]))

    # Compatibility fallback for older fixtures/assets whose property tag header
    # differs from ARK's compact layout. This remains heuristic and callers keep
    # the existing low confidence on unresolved compound values.
...
def cdo_declared_value_size(export_data: bytes, pos: int, end: int, type_name: str) -> int:
    if type_name == "StructProperty":
        ...
        candidates = (pos + 32, pos + 20, pos + 16, pos + 44)
    else:
...
def cdo_value_offset(export_data: bytes, block: dict[str, object], type_name: str) -> tuple[int, int]:
    if block.get("tag_layout") in {
        "ark_compact",
        "ark_compact_guid_marker",
        "ue5_property_type_name",
    }:
        return int(block.get("value_offset") or 0), int(block.get("declared_size") or 0)
    pos = int(block.get("offset") or 0)
    end = int(block.get("end") or len(export_data))
...
def _parse_cdo_array_value(
    ...
    """Decode an exactly bounded ARK ArrayProperty, including tagged structs."""

    tag_layout = str(block.get("tag_layout") or "")
    if tag_layout not in {
        "ark_compact",
        "ark_compact_guid_marker",
        "ue5_property_type_name",
    }:
    ...
    if declared_size < 4 or value_end > len(export_data):
        return None
    count = _read_i32(export_data, value_offset)
    if count < 0 or count > 1_000_000:
        return None
    inner_type = str(block.get("inner_type") or "")
    ...
    if inline_soft_object_paths:
        ...
        for index in range(count):
            element_start = cursor
            if inner_type in {"ObjectProperty", "SoftObjectProperty", "IntProperty"}:
                ...
                value = _read_f64(export_data, cursor)
            elif inner_type == "BoolProperty":
                value = bool(export_data[cursor])
            else:
                fname_value = _fname_at(export_data, cursor, names)
                if not fname_value:
                    return None
                value = fname_value[0]
            cursor += width
        ...
        sequence_parser = (
            _ue5_cdo_property_sequence
            if tag_layout == "ue5_property_type_name"
            else (
                _ark_guid_cdo_property_sequence
        ...
        for index in range(count):
            ...
            if not terminated or cursor_after <= cursor:
                return None
            nested_properties = [
                parse_cdo_property_value(export_data, nested, names, imports, exports, soft_object_paths)
            ...
            elements.append(
                {
                    ...
                    "raw_offsets": raw_offsets(element_start, cursor_after),
                }
            )
            cursor = cursor_after
        if cursor != value_end:
...
def parse_cdo_property_value(
    ...
    item: dict[str, object] = {
        ...
        "array_index": int(block.get("array_index") or 0),
    }
    if block.get("tag_layout"):
        item["tag_layout"] = block["tag_layout"]
    if block.get("inner_type"):
        item["inner_type"] = block["inner_type"]
    value_end = min(len(export_data), end)
    if declared_size > 0:
        value_end = min(value_end, value_offset + declared_size)
    try:
        if type_name == "DoubleProperty" and value_offset + 8 <= value_end:
            item["value"] = _read_f64(export_data, value_offset)
        elif type_name == "FloatProperty" and value_offset + 4 <= value_end:
            item["value"] = _read_f32(export_data, value_offset)
        elif type_name == "IntProperty" and value_offset + 4 <= value_end:
            ...
            value = _fname_at(export_data, value_offset, names) if value_offset + 8 <= value_end else None
            item["value"] = value[0] if value else ""
        elif type_name == "ObjectProperty" and value_offset + 4 <= value_end:
            ...
            if _valid_package_index(ref, imports, exports):
                ...
                item["value"] = None
                item["object"] = ""
                item["value_status"] = "NOT_RECOVERED"
                item["error"] = f"PackageIndex {ref} is outside package maps"
        elif type_name == "SoftObjectProperty" and value_offset + 4 <= value_end:
            ...
            if soft_path:
                item["value"] = str(soft_path.get("object_path") or "")
                ...
                item["object"] = ""
                item["value_status"] = "NOT_RECOVERED"
                item["error"] = f"SoftObjectPath index {path_index} is outside path map"
        elif type_name == "StructProperty":
            struct_name = str(block.get("struct") or "")
            if not struct_name:
    ...
    if item.get("error"):
        ...
        array_parse = item.get("array_parse", {})
        item["confidence"] = "medium" if isinstance(array_parse, dict) and array_parse.get("parsed") else "low"
    elif not block.get("tag_layout"):
        item["confidence"] = "low"
    elif type_name == "MapProperty" or (isinstance(value, dict) and value.get("parsed") is False):
        item["confidence"] = "low"
...
def read_uasset_class_defaults(package: dict[str, object], asset_name: str) -> dict[str, object]:
    ...
    for block in cdo_property_tag_blocks(export_data, names):
        ...
        array_index = int(prop.get("array_index") or 0)
        variable_name = property_name if array_index == 0 else f"{property_name}[{array_index}]"
...
def valid_blueprint_node_ref(ref: int, exports: list[dict[str, object]]) -> bool:
    node_index = package_index_to_export_index(ref)
    if node_index is None or not (0 <= node_index < len(exports)):
        return False
    node_export = exports[node_index]
    return isinstance(node_export, dict) and is_blueprint_node_export(node_export)


def normalize_blueprint_node_refs(raw_refs: Iterable[int], exports: list[dict[str, object]]) -> list[int]:
    refs: list[int] = []
    seen: set[int] = set()
    for ref in raw_refs:
        value = int(ref)
        if value in seen or not valid_blueprint_node_ref(value, exports):
            continue
        refs.append(value)
        seen.add(value)
    return refs
...
def _is_pin_name_candidate(name: str) -> bool:
    ...
    if name != name.strip():
        return False
    lowered = name.lower()
    if lowered in PIN_CATEGORY_NAMES or lowered in INTERNAL_PIN_PROPERTY_NAMES or lowered in {"none", "axis", "local pc"}:
        return False
    if name.startswith(("/Game", "/Script", "/Engine")) or "/" in name or "\\" in name:
        return False
    if name.startswith(("K2Node_", "EdGraph", "Default__")):
        return False
    if lowered.startswith(("callfunc_", "conv_")):
        return False
    if len(name) > 80:
        return False
...
def extract_default_object_name(
    ...
    def useful_name(name: str) -> bool:
        if not name:
            return False
        if name in {"StandardMacros", "Texture", "Texture2D", "Material", "Object", "Class", "Blueprint"}:
            return False
        if name.startswith(("/Engine/EditorBlueprintResources/", "K2Node_", "EdGraph", "Default__")):
            return False
        return True
...
def _printable_fstring_candidates(region: bytes, *, limit: int = 512) -> list[str]:
    values: list[str] = []
    seen: set[str] = set()
    upper = max(0, min(len(region) - 4, limit))
    for pos in range(0, upper):
        try:
            value, _next = _read_fstring(region, pos)
        except Exception:
            continue
        if not value or value in seen or len(value) > 500:
            continue
...
def exported_pin_id(pin_ref: int, pin_export: dict[str, object]) -> str:
    return str(pin_export.get("display_name") or pin_export.get("object_name") or f"EdGraphPin_{pin_ref}")


def valid_pin_export_ref(ref: int, exports: list[dict[str, object]]) -> bool:
    pin_index = package_index_to_export_index(ref)
    if pin_index is None or not (0 <= pin_index < len(exports)):
        return False
    pin_export = exports[pin_index]
    return isinstance(pin_export, dict) and str(pin_export.get("class_name") or "") == "EdGraphPin"


def parse_exported_pin_name(properties: dict[str, dict[str, object]], pin_export: dict[str, object]) -> str:
    value = property_scalar(properties, "PinName")
    if isinstance(value, str) and value:
        return value
    return str(pin_export.get("display_name") or pin_export.get("object_name") or "")


def parse_exported_pin_direction(
    properties: dict[str, dict[str, object]],
    pin_data: bytes,
    names: list[str],
    *,
    ...
    if isinstance(block, dict):
        start = int(block.get("offset") or 0)
        end = int(block.get("end") or len(pin_data))
        region = pin_data[start:end]
        for _pos, name in _all_fname_candidates(region, names, 0, len(region)):
            if name.startswith("EGPD_"):
                return name
    return infer_pin_direction(pin_name, category, node_type)


def parse_exported_pin_category(
    properties: dict[str, dict[str, object]],
    pin_data: bytes,
    names: list[str],
) -> tuple[str, str, str, bytes]:
    block = properties.get("PinType", {})
    if not isinstance(block, dict):
        return "", "", "None", b""
    start = int(block.get("offset") or 0)
    end = int(block.get("end") or len(pin_data))
    region = pin_data[start:end]
    strings = _printable_fstring_candidates(region, limit=768)
    category = ""
    subcategory = ""
    for value in strings:
        lowered = value.lower()
        if not category and lowered in PIN_CATEGORY_NAMES:
            category = lowered
            continue
        if category and not subcategory and _is_pin_name_candidate(value):
            subcategory = value
            break
    if not category:
        for _pos, name in _all_fname_candidates(region, names, 0, len(region)):
            lowered = name.lower()
            if lowered in PIN_CATEGORY_NAMES:
                category = lowered
                break
    return category, subcategory, infer_pin_container_type(region, names), region


def parse_exported_pin_default(properties: dict[str, dict[str, object]], pin_data: bytes) -> str:
    for key in ("DefaultValue", "AutogeneratedDefaultValue", "DefaultTextValue"):
        value = property_scalar(properties, key)
        if isinstance(value, str) and value:
            return value
    return extract_pin_default_value(pin_data)


def exact_guid_property(
    properties: dict[str, dict[str, object]],
    property_name: str,
) -> str:
    """Return an exact, non-zero FGuid property and reject all other layouts."""

    item = properties.get(property_name, {})
    if not isinstance(item, dict):
        return ""
    struct_parse = item.get("struct_parse")
...
def parse_exported_pin_links(
    properties: dict[str, dict[str, object]],
    *,
    pin_owner_by_ref: dict[int, dict[str, object]],
) -> list[dict[str, object]]:
    links: list[dict[str, object]] = []
    for target_pin_ref in property_object_refs(properties, "LinkedTo"):
        owner = pin_owner_by_ref.get(target_pin_ref, {})
        target_node = str(owner.get("node_name") or "")
        target_pin_key = str(owner.get("pin_id") or "")
        target_pin_name = str(owner.get("pin_name") or "")
        node_package_index = int(owner.get("node_package_index") or 0)
        target_pin_id_is_exact = owner.get("pin_id_authority") == "EXACT"
        target_pin_id = target_pin_key if target_pin_id_is_exact else ""
        link: dict[str, object] = {
            "target_node": target_node,
            "target_pin_id": target_pin_id,
            "target_pin": target_pin_name,
            "target_package_index": node_package_index,
            "target_pin_package_index": target_pin_ref,
            "source_offset": 0,
            "target_pin_id_candidates": (
                [target_pin_id] if target_pin_id_is_exact else []
            ),
            "source": "uasset_exported_pin_linked_to",
            "confidence": "high" if target_pin_id_is_exact else "medium",
            "resolution_status": (
                "resolved_pin"
                if target_node and target_pin_id and target_pin_id_is_exact
                else "target_pin_identity_unavailable"
                if target_node and target_pin_key
                else "unresolved"
            ),
            "resolution_method": (
                "exact_exported_pin_linked_to"
                if target_node and target_pin_id and target_pin_id_is_exact
                else "export_object_reference_only"
                if target_node and target_pin_key
                else "unresolved"
            ),
        }
        if target_pin_id_is_exact:
            link["target_pin_id_authority"] = "EXACT"
        elif target_pin_key:
            link["target_pin_internal_key"] = target_pin_key
        links.append(link)
    return links


def parse_exported_pin_object(
    package: dict[str, object],
    pin_ref: int,
    *,
    ...
    graph_refset: set[int],
    pin_owner_by_ref: dict[int, dict[str, object]],
) -> tuple[PinInfo | None, list[str]]:
    names = package.get("names", [])
    imports = package.get("imports", [])
    exports = package.get("exports", [])
    if not isinstance(names, list) or not isinstance(imports, list) or not isinstance(exports, list):
        return None, ["Package maps were not available for exported pin parsing."]
    pin_index = package_index_to_export_index(pin_ref)
    if pin_index is None or not (0 <= pin_index < len(exports)):
        return None, [f"Pins array referenced unsupported package index {pin_ref}."]
    pin_export = exports[pin_index]
    if not isinstance(pin_export, dict) or str(pin_export.get("class_name") or "") != "EdGraphPin":
        return None, [f"Pins array referenced non-pin export {pin_ref}."]
    pin_data = export_data_bytes(package, pin_export)
    if not pin_data:
        return None, [f"{exported_pin_id(pin_ref, pin_export)}: Serialized pin data is not available."]
    ...
    default_object = extract_default_object_name(pin_type_region or pin_data, imports, exports, graph_refset)
    default_value = parse_exported_pin_default(properties, pin_data)
    native_pin_id = exact_guid_property(properties, "PinId")
    pin_guid = exact_guid_property(properties, "PersistentGuid")
    links = parse_exported_pin_links(properties, pin_owner_by_ref=pin_owner_by_ref)
    pin_id = native_pin_id or exported_pin_id(pin_ref, pin_export)
    pin_type = {
    ...
    warnings = [str(warning) for warning in property_warnings if str(warning)]
    if not pin_name:
        warnings.append("PinName was not decoded from exported EdGraphPin data.")
    if not category:
        warnings.append("PinType category was not decoded from exported EdGraphPin data.")
    pin = PinInfo(
        id=pin_id,
        name=pin_name or pin_id,
        direction=direction,
        category=category,
        ...
        linked_to_raw=" ".join(str(link.get("target_node") or link.get("target_pin_package_index") or "") for link in links),
        links=links,  # type: ignore[arg-type]
        source="uasset_exported_pin_object",
        confidence="high" if pin_name and category else "medium",
        warnings=warnings,
        raw_offsets=raw_offsets(
            int((pin_export.get("serial_location") or {}).get("offset") or 0)
            if isinstance(pin_export.get("serial_location"), dict)
            else 0,
            int((pin_export.get("serial_location") or {}).get("offset") or 0)
            + int((pin_export.get("serial_location") or {}).get("size") or 0)
            if isinstance(pin_export.get("serial_location"), dict)
            else 0,
        ),
        resolution={
            ...
            "native_pin_id_method": (
                "exact_struct_value" if native_pin_id else "UNAVAILABLE"
            ),
...
def parse_exported_pin_objects(
    ...
    *,
    ...
    if not isinstance(exports, list):
        return [], ["Package exports were not available for exported pin parsing."]
    pin_refs = [ref for ref in property_object_refs(properties, "Pins") if valid_pin_export_ref(ref, exports)]
    if not pin_refs:
        return [], []
    pins: list[PinInfo] = []
    warnings: list[str] = []
    for pin_ref in pin_refs:
        pin, pin_warnings = parse_exported_pin_object(
            package,
            pin_ref,
        ...
        if pin:
            pins.append(pin)
    if len(pins) < len(pin_refs):
        warnings.append(f"Recovered {len(pins)} of {len(pin_refs)} exported EdGraphPin objects.")
    return pins, warnings


def build_graph_pin_owner_index(package: dict[str, object], node_refs: Iterable[int]) -> dict[int, dict[str, object]]:
    names = package.get("names", [])
    imports = package.get("imports", [])
    exports = package.get("exports", [])
    if not isinstance(names, list) or not isinstance(imports, list) or not isinstance(exports, list):
    ...
    for node_ref in node_refs:
        node_index = package_index_to_export_index(int(node_ref))
        if node_index is None or not (0 <= node_index < len(exports)):
            continue
        node_export = exports[node_index]
        ...
        if not node_data:
            continue
        node_properties, _warnings = parse_export_properties(node_data, names, imports, exports)
        node_name = str(node_export.get("display_name") or node_export.get("object_name") or "")
        for pin_ref in property_object_refs(node_properties, "Pins"):
            if not valid_pin_export_ref(pin_ref, exports):
                continue
            pin_index = package_index_to_export_index(pin_ref)
            ...
            pin_export = exports[pin_index]
            pin_data = export_data_bytes(package, pin_export)
            pin_properties, _pin_warnings = parse_export_properties(pin_data, names, imports, exports)
            native_pin_id = exact_guid_property(pin_properties, "PinId")
            owners[pin_ref] = {
                "node_package_index": int(node_ref),
                "node_name": node_name,
                "pin_package_index": pin_ref,
                "pin_id": native_pin_id or exported_pin_id(pin_ref, pin_export),
                "pin_id_authority": (
                    "EXACT" if native_pin_id else "UNAVAILABLE"
                ),
                "pin_name": parse_exported_pin_name(pin_properties, pin_export),
            }
    return owners
...
def parse_custom_pins(
    ...
    *,
    ...
    pin_owner_by_ref: dict[int, dict[str, object]] | None = None,
) -> tuple[list[PinInfo], list[str]]:
    warnings: list[str] = []
    exported_pins, exported_warnings = parse_exported_pin_objects(
        package,
        properties,
        node_export=node_export,
        graph_refset=graph_refset,
        pin_owner_by_ref=pin_owner_by_ref or {},
    )
    if exported_pins:
        return exported_pins, exported_warnings
        ...
        for pos in range(name_pos, max(name_pos, region_end - 4)):
            ...
            if value in graph_refset and value != self_package_index:
                ...
                if 0 <= target_index < len(exports):
                    ...
                    marker = (value, target_pin_id or target_name)
...
def node_info_from_export(
    *,
    ...
    return NodeInfo(
        index=index,
        package_index=int(node_export.get("package_index") or 0),
        class_name=class_name,
...
def graph_coverage(nodes: list[NodeInfo]) -> dict[str, object]:
    ...
    exact_links = sum(
        ...
        if pin_has_authoritative_native_id(pin)
        and link.get("target_pin_id_authority") == "EXACT"
        and str(link.get("resolution_method") or "")
        in {
            "exact_existing_target_pin_id",
...
def is_complete_empty_graph(
    ...
    *,
    ...
    if len(nodes) != 1:
        return False
    node = nodes[0]
    if graph_type == "Function" and node.class_name == "K2Node_FunctionEntry" and node.link_count == 0:
        return True
...
def _link_uses_uasset_identity(
    source_pin: PinInfo,
    link: dict[str, object],
) -> bool:
    link_source = str(link.get("source") or link.get("link_source") or "")
    return source_pin.source.startswith("uasset_") or link_source.startswith(
        "uasset_"
    )


def _target_pin_candidate_ids(link: dict[str, object]) -> list[str]:
    raw_candidates = link.get("target_pin_id_candidates", [])
    ...
    for raw_candidate in raw_candidates:
        if isinstance(raw_candidate, dict):
            candidate = str(
                ...
                or raw_candidate.get("native_pin_id")
                or ""
            )
        else:
...
def _unique_nodes(nodes: Iterable[NodeInfo]) -> list[NodeInfo]:
...
def _mark_link_resolution(
    ...
    *,
    ...
    link.pop("target_pin_id_authority", None)
    link.pop("target_node_guid", None)
    if confidence:
...
def resolve_graph_link_target_pins(nodes: list[NodeInfo]) -> dict[str, object]:
    by_name: dict[str, list[NodeInfo]] = defaultdict(list)
    by_package_index: dict[int, list[NodeInfo]] = defaultdict(list)
    authoritative_pins: dict[str, list[tuple[NodeInfo, PinInfo]]] = defaultdict(
        list
    )
    for node in nodes:
        if node.name:
            by_name[node.name].append(node)
        if node.package_index:
            by_package_index[node.package_index].append(node)
        for pin in node.pins:
            if pin_has_authoritative_native_id(pin):
                authoritative_pins[pin.id].append((node, pin))
    ...
    def record_unresolved(
        ...
        *,
        ...
        nonlocal unresolved, ambiguous
        _mark_link_resolution(
            link,
            status=status,
            method=method,
            confidence="medium",
    ...
    def target_nodes_for(link: dict[str, object]) -> list[NodeInfo]:
        target_name = str(link.get("target_node") or "")
        try:
            package_index = int(link.get("target_package_index") or 0)
        except (TypeError, ValueError):
            package_index = 0
        candidates = (
            _unique_nodes(by_package_index.get(package_index, []))
            if package_index
            else []
        )
        if candidates and target_name:
            named = [node for node in candidates if node.name == target_name]
            if named:
                candidates = named
        if not candidates:
            candidates = _unique_nodes(by_name.get(target_name, []))
        return candidates

    def exact_target_match(
        pin_id: str,
        target_nodes: list[NodeInfo],
    ) -> tuple[tuple[NodeInfo, PinInfo] | None, str]:
        matches = authoritative_pins.get(pin_id, [])
        if len(matches) > 1:
            return None, "ambiguous_target_pin_identity"
        if not matches:
            return None, "target_pin_identity_unavailable"
        match = matches[0]
        if all(match[0] is not node for node in target_nodes):
            return None, "target_pin_identity_mismatch"
        return match, ""

    for source_node in nodes:
        for source_pin in source_node.pins:
            for link in source_pin.links:
                candidate_ids = _target_pin_candidate_ids(link)
                if (
                    str(link.get("resolution_method") or "")
                    == "exact_target_pin_id_candidate"
                    and str(link.get("target_pin_id") or "") in candidate_ids
                ):
                    link["target_pin_id"] = ""
                    link.pop("target_pin", None)
                    link.pop("target_pin_id_authority", None)
                    link.pop("target_node_guid", None)

                uses_uasset_identity = _link_uses_uasset_identity(
                    source_pin,
                    link,
                )
                requested_pin_id = str(link.get("target_pin_id") or "")
                had_non_authoritative_target_key = (
                    uses_uasset_identity
                    and requested_pin_id
                    and link.get("target_pin_id_authority") != "EXACT"
                )
                if had_non_authoritative_target_key:
                    link["target_pin_internal_key"] = requested_pin_id
                    link["target_pin_id"] = ""
                    requested_pin_id = ""

                source_matches = (
                    authoritative_pins.get(source_pin.id, [])
                    if pin_has_authoritative_native_id(source_pin)
                    else []
                )
                ...
                target_nodes = target_nodes_for(link)
                if not target_nodes:
                    method = "cross_graph_or_missing_node" if link.get("target_node") else "unresolved"
                    record_unresolved(link, method, method)
                    continue
                ...
                requested_pin_id = str(link.get("target_pin_id") or "")
                requested_id_is_authoritative = bool(requested_pin_id) and (
                    not uses_uasset_identity
                    or link.get("target_pin_id_authority") == "EXACT"
                )
                target_match: tuple[NodeInfo, PinInfo] | None = None
                exact_method = ""
                if requested_id_is_authoritative:
                    target_match, failure_status = exact_target_match(
                        requested_pin_id,
                        target_nodes,
                    )
                    if failure_status == "ambiguous_target_pin_identity":
                        record_unresolved(
                            link,
                            failure_status,
                            failure_status,
                    ...
                    if failure_status:
                        record_unresolved(
                            link,
                            failure_status,
                            "exact_target_pin_id_not_found",
                        )
                        continue
                    exact_method = "exact_existing_target_pin_id"
                elif requested_pin_id:
                    if uses_uasset_identity:
                        link["target_pin_internal_key"] = requested_pin_id
                        link["target_pin_id"] = ""
                    record_unresolved(
                        link,
                    ...
                    candidate_method = str(
                        link.get("target_pin_id_candidate_method") or ""
                    )
                    candidates_are_authoritative = (
                        not uses_uasset_identity
                        or candidate_method
                        in AUTHORITATIVE_TARGET_PIN_CANDIDATE_METHODS
                    )
                    if not candidates_are_authoritative:
                        record_unresolved(
                            link,
                            "target_pin_identity_unavailable",
                        ...
                        continue
                    matched: list[tuple[NodeInfo, PinInfo]] = []
                    failure_status = ""
                    for candidate_id in candidate_ids:
                        match, candidate_failure = exact_target_match(
                            candidate_id,
                            target_nodes,
                        )
                        if candidate_failure == "ambiguous_target_pin_identity":
                            failure_status = candidate_failure
                            break
                        if match is not None and all(
                            match[1] is not prior[1] for prior in matched
                        ):
                            matched.append(match)
                    if failure_status or len(matched) > 1:
                        record_unresolved(
                            link,
                    ...
                    if not matched:
                        record_unresolved(
                            link,
                            "target_pin_identity_unavailable",
                            "exact_target_pin_id_candidate_not_found",
                        )
                        continue
                    target_match = matched[0]
                    exact_method = "exact_target_pin_id_candidate"

                if target_match is not None:
                    target, target_pin = target_match
                    ...
                    link["resolution_method"] = exact_method
                    link["target_pin_id_authority"] = "EXACT"
                    if exact_method == "exact_target_pin_id_candidate":
                ...
                if candidates:
                    ...
                    link["resolution_method"] = "heuristic_direction_category"
                    link["confidence"] = confidence_floor(str(link.get("confidence") or "medium"), "medium")
                    heuristic += 1
                    method_counts["heuristic_direction_category"] += 1
                else:
                    link["resolution_status"] = "node_resolved_pin_unknown"
                    link["resolution_method"] = "node_only"
                    unresolved += 1
                    method_counts["node_only"] += 1
    return {
        "resolved_pin": exact,
...
def build_pin_link_payload(read_payload: dict[str, object]) -> dict[str, object]:
    ...
    for graph in read_payload.get("graphs", []):
        ...
        graph_method_counts: Counter[str] = Counter()
        for link in graph_payload.get("links", []):
            ...
            if link_status:
                ...
                status = "unresolved"
            method = str(link.get("resolution_method") or "")
            if not method:
                if status == "resolved_pin" and str(link.get("target_pin_id") or ""):
                    method = "exact_existing_target_pin_id"
                elif status == "resolved_pin_heuristic":
                    method = "heuristic_direction_category"
                elif status == "node_resolved_pin_unknown":
                    method = "node_only"
                elif status == "cross_graph_or_missing_node":
                    method = "cross_graph_or_missing_node"
                else:
                    method = "unresolved"
            row = {
                ...
                "source_pin_id_authority": link.get(
        ...
        graph_summaries.append(
            {
                ...
                "method_counts": dict(sorted(graph_method_counts.items())),
                "links": links,
...
def parse_graph_export_payload(
    ...
    *,
    ...
    if raw_node_refs and ignored_node_ref_count:
        graph_warnings.append(f"Ignored {ignored_node_ref_count} non-node or duplicate refs from EdGraph.Nodes.")
    if not node_refs:
    ...
    pin_owner_by_ref = build_graph_pin_owner_index(package, node_refs)
    nodes: list[NodeInfo] = []
    ...
    if empty_graph_complete and isinstance(payload.get("diagnostics"), dict):
        payload["diagnostics"]["confidence_level"] = confidence
        payload["diagnostics"]["missing_entry_points"] = False
    graph_record = {
        "graph": graph_name,
        "graph_type": graph_type,
...
def current_uasset_graph_payload_files(asset_dir: Path) -> list[Path]:
        ...
        if isinstance(files, list):
            for item in files:
                ...
                try:
                    ...
                    continue
                try:
                    resolved.relative_to(graphs_dir.resolve())
                except ValueError:
                    continue
...
def render_pin_link_report(payload: dict[str, object]) -> str:
    ...
    if isinstance(resolution_counts, dict) and resolution_counts:
        ...
        lines.append("| none | 0 | - |")
    lines.extend(["", "## Resolution Methods", "", "| Method | Count | Meaning |", "| --- | ---: | --- |"])
    method_meanings = {
        "exact_existing_target_pin_id": "target PinId was already present on the link",
        "exact_target_pin_id_candidate": "target PinId was recovered from explicit candidate bytes and matched a target pin",
        "exact_target_node_guid_and_pin_id": "target node guid and target PinId were both exact",
        "exact_target_node_pin_name": "target node and pin name were exact",
        "heuristic_direction_category": "target pin was selected by direction/category fallback",
        "heuristic_boundary_synthesis": "target pin was synthesized for a graph boundary",
        "node_only": "target node resolved but target pin remains unknown",
        "cross_graph_or_missing_node": "target node was not present in this graph payload",
        "unresolved": "no target node or pin could be recovered",
    }
    method_counts = summary.get("method_counts", {})
    if isinstance(method_counts, dict) and method_counts:
        for method, count in sorted(method_counts.items()):
            lines.append(f"| {method} | {count} | {method_meanings.get(str(method), '')} |")
    else:
        lines.append("| none | 0 | - |")
...
def compare_uasset_with_clipboard(asset_dir: Path, *, keywords: list[str] | None = None) -> dict[str, object]:
    ...
    uasset_files = current_uasset_graph_payload_files(asset_dir)
...
def _write_legacy_uasset_graph_read_files(asset_path: str, capture_root: Path, payload: dict[str, object]) -> dict[str, str]:
    asset_name = str(payload.get("asset_name") or asset_name_from_object_path(asset_path) or "Blueprint")
    asset_dir = capture_root / safe_filename(asset_name)
    asset_dir.mkdir(parents=True, exist_ok=True)
    graphs_dir = asset_dir / "graphs_from_uasset"
    ...
    graph_manifest_path = asset_dir / "graphs_from_uasset_manifest.json"
            ...
            manifest_files.append(
                {
                    ...
                    "status": graph.get("status", ""),
                    "confidence": graph.get("confidence", ""),
                    "path": str(graph_file.relative_to(asset_dir)).replace("\\", "/"),
                }
            )
            graph_files += 1
        if str(graph.get("status") or "") != "complete":
        ...
        json.dumps(
            {
                ...
                "generated": payload.get("generated", ""),
                "asset_name": payload.get("asset_name", ""),
                "source_graph_count": len([graph for graph in payload.get("graphs", []) if isinstance(graph, dict)]),
...
def write_uasset_graph_read_files(
    asset_path: str,
    capture_root: Path,
    ...
    *,
    ...
    Indexed mode writes the normalized store directly from ``payload`` and
    deliberately leaves every pre-existing legacy artifact untouched.
    """

    from .artifact_modes import normalize_artifact_mode, write_evidence_artifacts_from_payload
    ...
    if mode in {"legacy", "dual"}:
        result_paths.update(_write_legacy_uasset_graph_read_files(asset_path, capture_root, payload))
    if mode in {"dual", "indexed"}:
        uasset_path = payload.get("uasset_path")
        if not uasset_path and isinstance(payload.get("package"), dict):
            uasset_path = payload["package"].get("uasset_path", "")
        evidence = write_evidence_artifacts_from_payload(
            asset_path,
            str(uasset_path or ""),
            payload,
            asset_dir,
        ...
        result_paths.update(
            {
                "evidence_database": str(evidence["database_path"]),
                "evidence_manifest": str(evidence["manifest_path"]),
                "agent_index": str(evidence["agent_index_path"]),
