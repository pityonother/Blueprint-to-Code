...
if (-not $HasPrecompiledBinary -and -not $CanCompileSourcePlugin -and -not $ForceSourceInstall) {
    ...
    Write-Host "The read-only Editor State Bridge also requires a compiled plugin module."
    Write-Host ""
...
Write-Host "  5. Confirm .arkdev-bridge\editor_state.json appears and updates automatically."
Write-Host "  6. Run python scripts\validate_arkdev_editor_bridge_snapshot.py."
