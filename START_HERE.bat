...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$root=(Resolve-Path -LiteralPath $env:BLUEPRINT_TO_CODE_LAUNCH_ROOT).Path.TrimEnd('\'); $serverScript=Join-Path $root 'scripts\blueprint_tool_server.py'; Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -and $_.CommandLine.IndexOf($serverScript, [StringComparison]::OrdinalIgnoreCase) -ge 0 } | ForEach-Object { try { Stop-Process -Id $_.ProcessId -Force -ErrorAction Stop } catch {} }"
set "BLUEPRINT_TO_CODE_LAUNCH_ROOT="
if defined BLUEPRINT_TO_CODE_NO_OPEN (
