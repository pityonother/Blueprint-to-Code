...
解析顺序是 owner/class、function name、signature/parameter hints、
PDB-qualified name、parent/inheritance。只有一个满足约束的候选才可
`CONFIRMED`。多个候选必须为 `AMBIGUOUS` 并保存全部候选；没有候选必须按
原因区分 `SOURCE_NOT_AVAILABLE` 与 `NOT_RECOVERED`。不得只按短函数名猜测。
